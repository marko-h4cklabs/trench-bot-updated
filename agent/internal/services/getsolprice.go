...
func FetchSOLPrice() (float64, error) {
	...
	log.Println("CoinGecko API failed. Switching to Binance API...")
...
func fetchWithRetries(url, mainKey, subKey string) (float64, error) {
	...
	for i := 0; i < maxRetries; i++ {
		resp, err := client.Get(url)
		if err != nil {
			log.Printf(" Attempt %d: API call failed: %v", i+1, err)
		...
		if err := json.Unmarshal(body, &data); err != nil {
			log.Printf("Attempt %d: Failed to parse JSON: %v", i+1, err)
			time.Sleep(time.Duration(math.Pow(2, float64(i))) * time.Second)
			continue
		}
		...
		log.Printf("Attempt %d: Price not found in response", i+1)
		time.Sleep(time.Duration(math.Pow(2, float64(i))) * time.Second)
	}
